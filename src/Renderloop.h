...
class Renderloop : public DXSample {
public:
	Renderloop(uint32_t width, uint32_t height, std::wstring name);
	Renderloop(Renderloop const&) = delete;
...
private:
	...
	std::unordered_map<std::string, MaterialInfo*> m_materials;
	...
	std::unique_ptr<DescriptorHeap> m_dynamicSrvHeap;
	std::unique_ptr<DescriptorHeap> m_shadowmap_dsvHeap;
	...
	std::unique_ptr<RasterShader> smShader;
	std::unique_ptr<DirectionalLight> sunLight;
